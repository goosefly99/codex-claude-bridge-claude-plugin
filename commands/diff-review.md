...
description: Neutral code review of the current diff using OpenAI Codex. Auto-classifies large diffs (8 files / 500 LOC defaults) and supports --background / --wait routing. For reviewing arbitrary files or folders (not a diff), use /codex:review.
argument_hint: "[--effort low|medium|high] [--background|--wait] [<git-ref>]"
allowed_tools: ["Bash", "Read"]
script: ${CLAUDE_PLUGIN_ROOT}/dist/codex/cli-diff-review.js
---

# /codex:diff-review

You are running the `codex-claude-bridge` neutral diff-review command. This is the non-adversarial sibling of `/codex:adversarial-diff-review`: same diff handling, same transport, same auto-classification — but a *neutral* system prompt without the 7-attack-surface taxonomy. Use it when the user wants a calm second-pair-of-eyes pass over a diff rather than a hostile audit.

For a neutral review of arbitrary files or folders (no git diff involved), use `/codex:review` instead.
...
- This is the *neutral* review. Do NOT inject the 7-attack-surface taxonomy here. That's `/codex:adversarial-diff-review` exclusively.
- The output of `/codex:diff-review` is unstructured prose. Closed-loop plan-mode integration is the adversarial command's job.
- All retries (429, 5xx) go through the transport's exponential-backoff layer; this command does not retry independently.
- If the user is asking about content outside the working diff, redirect them to `/codex:review <path...>` rather than running this command on unrelated changes.
